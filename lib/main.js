...
export default {
  ...
  activate (_state) {
    ...
    subscriptions.add(atom.commands.add('atom-text-editor', {
      'bacadra-word-map:auto' : () => this.convert(0, 1),
      'bacadra-word-map:selected' : () => this.convert(2),
      'bacadra-word-map:fixed-1' : () => this.convert(1, 1),
      'bacadra-word-map:fixed-2' : () => this.convert(1, 2),
      'bacadra-word-map:fixed-3' : () => this.convert(1, 3),
      'bacadra-word-map:fixed-4' : () => this.convert(1, 4),
      'bacadra-word-map:fixed-5' : () => this.convert(1, 5),
    }));
  },
  ...
  convert(mode=0, wLen=1) {
    ...
    for (var selection of selections) {
      ...
      if ((mode==0 && selection.isEmpty()) || mode==1) {
        ...
        if (word=='') {
          if (atom.config.get('bacadra-word-map.silent')!=true) {
      ...
      customDmap = eval('('+atom.config.get("bacadra-word-map.customDmap")+')')

      if (word in customDmap) {
        selection.insertText(customDmap[word])

      } else if (word in dmap) {
        selection.insertText(dmap[word])

      } else if (atom.config.get('bacadra-word-map.silent')!=true) {
